...
kubectl-apply:
	...
	sed -e "s|{{IMAGE_NAME}}|$(TAG)|g" k8s/deployment.yaml | kubectl apply --namespace=production -f -
	kubectl apply --namespace=production -f k8s/service.yaml
	kubectl apply --namespace=production -f k8s/ingress.yaml
